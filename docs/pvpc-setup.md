...
23 entidades registradas. Las principales:
...
| `sensor.esios_current_price_level` | Nivel de precio (very_high/high/neutral/low/very_low) | — |
...
`docker cp` solo funciona con destinos dentro de `/config/`. No usar `/tmp/` como destino intermedio dentro del container.
